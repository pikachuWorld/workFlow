 ...
 * @LastEditTime: 2022-11-26 14:36:49
-->

<template>

    <div class="">
        <h2><a href="" target="_blank">D3演示 例1-柱状图</a></h2>
        <div id="force"></div>
    </div>
...
export default {
    ...
    methods: {
        drawZ() {
            ...
            // 处理数据
            const nameArr = dataArr.map((item) => {
                return item.name
            })
            const valueArr = dataArr.map((item) => {
                return item.value
            })
            //指定容器的宽高
            let padding = { top: 20, right: 20, bottom: 20, left: 20 },
                width = 300 - padding.right - padding.left,
                height = 300 - padding.bottom - padding.top
            //x,y轴比例尺
            let xscale = d3
                .scaleBand()
                .domain(nameArr)
